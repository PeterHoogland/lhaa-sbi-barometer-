...
**Vlag:** `uncertainty_flag` op de intervalbreedte als fractie van de 0–100-schaal: low < 0,10 ≤ medium ≤ 0,20 < high. De vlag wordt berekend op de **gepubliceerde** (afgeronde) grenzen, zodat een lezer die de regel op de JSON-velden toepast altijd dezelfde vlag krijgt als de engine. Structurele overrides: minder dan 30 referentiepunten voor het percentiel → high (`thin_reference`); nul gescoorde indicatoren → high met volledig bereik en zónder composiet-CI (`no_scored_indicators`, composite_ci_95 = null). Weergave (productkeuze Peter, 2026-06-13, driemaal aangescherpt): de UI toont **altijd één getal op 100** en het niveauwoord is **altijd een schaalwoord laag → extreem** (P50/P70/P90-banden; de top-10%-band heet "extreem", zelfde register als de indicator-states). Onzekerheid is geen niveau en krijgt dus geen eigen kicker én geen waarschuwingszin; ze blijft zichtbaar via de 90%-band in de meter en de sobere bandbreedte-regel onder het cijfer, in élke vlagtoestand dezelfde vorm (bij `thin_reference` "indicatief" in plaats van de "90% zeker"-claim: het interval dekt die onzekerheid juist niet). De oorspronkelijke B3-weergave (bereik in plaats van getal, kicker "ONZEKER" bij high) en de tussenvorm met de zin "De meting is vandaag onzeker: …" bij vlag high zijn hiermee vervangen; de data-velden zijn ongewijzigd.
