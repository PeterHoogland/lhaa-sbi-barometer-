...
**Vlag:** `uncertainty_flag` op de intervalbreedte als fractie van de 0–100-schaal: low < 0,10 ≤ medium ≤ 0,20 < high. De vlag wordt berekend op de **gepubliceerde** (afgeronde) grenzen, zodat een lezer die de regel op de JSON-velden toepast altijd dezelfde vlag krijgt als de engine. Structurele overrides: minder dan 30 referentiepunten voor het percentiel → high (`thin_reference`); nul gescoorde indicatoren → high met volledig bereik en zónder composiet-CI (`no_scored_indicators`, composite_ci_95 = null). Weergave (productkeuze Peter, 2026-06-13): de UI toont **altijd één getal op 100**; de onzekerheid blijft zichtbaar via de band in de meter, de kicker "ONZEKER" bij high, en de bandbreedte-regel onder het cijfer (bij `thin_reference` zonder de "90% zekerheid"-claim: het interval dekt die onzekerheid juist niet). De oorspronkelijke B3-weergave (bereik in plaats van getal bij high) is hiermee vervangen; de data-velden zijn ongewijzigd.
