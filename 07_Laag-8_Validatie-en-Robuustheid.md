...
**Vlag:** `uncertainty_flag` op de intervalbreedte als fractie van de 0–100-schaal: low < 0,10 ≤ medium ≤ 0,20 < high. De vlag wordt berekend op de **gepubliceerde** (afgeronde) grenzen, zodat een lezer die de regel op de JSON-velden toepast altijd dezelfde vlag krijgt als de engine. Structurele overrides: minder dan 30 referentiepunten voor het percentiel → high (`thin_reference`); nul gescoorde indicatoren → high met volledig bereik en zónder composiet-CI (`no_scored_indicators`, composite_ci_95 = null). Weergave (productkeuze Peter, 2026-06-13/14, meermaals aangescherpt): de UI toont **altijd één getal op 100**. Het niveauwoord volgt de percentielband met een **relatieve (anomalie-)woordenschat**: **RUSTIG < 50 ≤ NORMAAL < 70 ≤ VERHOOGD < 90 ≤ UITZONDERLIJK**. Relatief, want het cijfer is een percentiel ("hoe ongewoon is vandaag t.o.v. normaal"), geen absolute meting; "RUSTIG/NORMAAL" sluiten aan op de CN-namen, "UITZONDERLIJK" op het indicator-woord (één register). De **mediaan (50)** is het zichtbare scharnier: RUSTIG (rustiger dan gewoonlijk) kantelt daar naar NORMAAL, óók op de kleurbalk (rustig 0-50 in lichter groen, normaal 50-70 in dieper groen; amber 70-90, rood alleen de top-10%). De campagne-/trigger-drempel blijft bewust op P70/P90 (pre-geregistreerd): de wéérgave reageert bij de mediaan, de campagne pas in de bovenste staart — een percentiel van 50 is per definitie een normale dag, dus daar alarmeren zou de helft van alle dagen "verhoogd" noemen.

**Band-bewuste kicker (Peter 14/6, IPCC-kalibratieprincipe):** kruist de gepubliceerde 90%-band een niveaugrens, dan toont de kicker het **bereik** ("RUSTIG TOT VERHOOGD") in plaats van één woord dat het eigen interval zou tegenspreken. Binnen één niveau blijft het één woord. Het hoofdcijfer blijft één getal op 100; alleen het woord wordt soms een bereik. De kicker leidt af van dezelfde afgeronde `ci_90`-grenzen die de lezer in de meter ziet (`scoreBand`/`BAND_LABEL`, gedeelde bron — geen drempel-duplicaat).

Onzekerheid is geen niveau en krijgt dus geen eigen kicker, geen waarschuwingszin en (sinds 14/6, Peter) geen zichtbare bandbreedte-regel meer; ze blijft zichtbaar als de **90%-band in de meter** (de primaire weergave) en wordt voor schermlezers als `sr-only`-tekst meegegeven (de meter is `aria-hidden`), zodat een score nooit zónder onzekerheid wordt uitgeleverd — het principe achter de v0.4-CI-uitbreiding blijft dus intact. Bij `thin_reference` luidt die tekst "indicatief bereik" in plaats van de "90% zeker"-claim (het interval dekt die onzekerheid juist niet). De percentielzin onder het cijfer benoemt expliciet de referentie: "hoger dan op X% van de dagen rond deze tijd van het jaar, gemeten over de laatste twee jaar" (de 24-maands-groundroot). De oorspronkelijke B3-weergave (bereik in plaats van getal, kicker "ONZEKER" bij high), de tussenvorm met de zin "De meting is vandaag onzeker: …", de zichtbare bandbreedte-regel, en de eerdere absolute woordenschat (LAAG/GEMIDDELD/VERHOOGD/EXTREEM) zijn hiermee vervangen; de data-velden zijn ongewijzigd.
...
**Uitbreiding naar de v0.4-kernlaag (2026-06-13, na adversariële review; gating-besluit Peter).** Dezelfde bootstrap-machinerie levert nu ook een CI rond de v0.4-score (`v04.uncertainty`, rond `percentile.lang`). Reden: zodra de campagne-koppeling live gaat, stuurt de v0.4-kern de publieke kop; zonder eigen CI zou het publiekscijfer dan zónder onzekerheidsweergave verschijnen — precies het cijfer dat de campagne aanstuurt, en in strijd met deze §13-bis-discipline (een score zonder band in een product dat overal elders een band toont, is een impliciete "nul onzekerheid"-claim). De v0.4-CI hertrekt per gescoorde kern-indicator de **lange** baseline (z_lang; die stuurt `percentile.lang`), aggregeert met de **kern-gewichten** `compositeMeting` (niet het equal-composiet, anders een maat-mismatch tussen band en getal) en neemt de percentielpositie binnen exact dezelfde `compositeMeting`-historie als de gepubliceerde score. Eigen deterministische seed (datum + ":v04") zodat de v0.2- en v0.4-trekkingen niet correleren. Dekking, vlag-drempels en `thin_reference`-/`no_scored_indicators`-overrides zijn identiek aan de v0.2-bootstrap hierboven. Hiermee vervalt de eerdere "black-out" (geen onzekerheidsweergave in live-modus): er bestaat geen toestand meer waarin een SBI-score zonder band publiek gaat. Dit is een uitbreiding van de NOG-niet-bevroren v0.4-laag (spec §8); bij de formele go-live van v0.4 als publieke kop hoort deze grootheid in het §4.1-amendement dat die go-live vastlegt.
