...
1. `runtime.ts` haalde de omgevings-dagsignalen (pollen I-D1-010, luchtkwaliteit I-D1-004) uit de historie ZONDER grens op `input.date`: `series[series.length - 1]` gaf altijd de LAATSTE rij, dus de waarde van vandaag. Bij het herbouwen van de reeks kreeg elke historische dag daardoor de pollen- en luchtwaarde van vandaag (lookahead/carry-back), waardoor de gepubliceerde reeks naar het niveau van vandaag werd getrokken en dagen kunstmatig gelijk werden. De regel ernaast (`computeBroadPressure(input.history, input.date)`) gaf de datum wel door; deze tak was vergeten.
2. `run.py` normaliseerde `observation_date` naar YYYY-MM-DD met takken voor lengte 7 en 10, maar niet voor lengte 4. Het jaarcijfer I-D2-001 ("2024", Verkeerscentrum-jaarrapport, bewust `imputed=True`) viel daardoor terug op de RUNDATUM. Gevolg in `app/data/history/I-D2-001.json`: 55 opeenvolgende "dagmetingen" van een jaarconstante, een geimputeerde waarde die zich als verse meting voordeed (harde regel 1). Erger nog: die 55 duplicaten kaapten de mediaan van het 60-maands recency-venster (16,39 -> 12,66) en pletten de MAD naar exact 0, waardoor de z van een grade-A indicator (gewicht 3) gedwongen op 0 stond. De indicator was de facto uitgeschakeld.
3. `healthcheck._parse_obs` accepteerde alleen "%Y-%m-%d" en "%Y-%m". Een jaarcijfer gaf `age=None`, waardoor de stale-tak nooit vuurde en de tolerantie van 400 dagen voor I-D2-001 per constructie onbereikbaar was.

**Beslissing:** (1) De reeks in `runtime.ts` wordt geklemd op `input.date`, dezelfde regel als `latestAtOrBefore` in `economic-pressure.ts`. (2) `run.py` krijgt een tak voor lengte 4 (jaarcijfer -> 1 januari), zodat de observatieperiode behouden blijft en de maand-deduplicatie weer grijpt. (3) `_parse_obs` accepteert "%Y". (4) De 55 vervuilde rijen in `app/data/history/I-D2-001.json` zijn verwijderd; de reeks staat terug op de maandconventie die de backfill schrijft (132 rijen, 2014-01-01 t/m 2024-12-01). GEEN methodologie-, weging- of indicatorwijziging: geen enkele definitie is aangeraakt, dit zijn correcties van code die niet deed wat ze beschreef.

**Geborgd:** Engine 211/211 (14 bestanden; 2 nieuwe regressietests in `hybrid-headline.test.ts` die de lookahead vastpinnen). Bewezen dat de tests de bug vangen: tegen de oude code faalt de test met "expected 200 to be 19", dus een scoredag pakte aantoonbaar een toekomstige waarde. `tsc --noEmit` schoon, alle 12 pipeline-suites groen. Effect gemeten door de reeks opnieuw te bouwen en te vergelijken met de live reeks over hetzelfde venster van 61 dagen: 50 dagen veranderen, de stilstand daalt van 32% naar 22% van de dagen, de gemiddelde dagbeweging stijgt van 2,15 naar 2,78 punt, en de gewraakte reeks 87/87/87 op 23-24-25 juli wordt 84/87/88. Het cijfer van VANDAAG verandert niet (88 voor en na), dus geen sprong voor bezoekers. De z van I-D2-001 gaat van het artefact 0 naar de correcte -0,674 (nagerekend: mediaan 16,3912, MAD 3,7285 over het 60-maandsvenster), waardoor het seizoenspercentiel van 61 naar 59 corrigeert.

**Nog open (bewust NIET meegenomen, vraagt een amendement):** de hitte-drempel `max(0, Tmax-30)` staat 87% van de dagen op nul (historisch 2-4% niet-nul per jaar), energie I-D3-002 plakt permanent tegen de winsor-kap van +3 (ruwe z 3,13), en de warm-up-loop in `generate-fixture.ts` geeft geen dagsignalen mee waardoor historische dagen met een andere formule rekenen dan vandaag. Alle drie raken §4.1 en horen niet in een bugfix-commit.
