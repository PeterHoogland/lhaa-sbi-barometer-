...
**Laag 2-bis — per run, referentie-audit (Peter, 2026-06-14):** na de verwerking reproduceert de engine elk dagpercentiel uit zijn eigen seizoensreferentie en weegt af of die referentie betrouwbaar is. Het blok `reference_audit` (in `latest.json`) draagt: reproduceerbaarheid (is het gepubliceerde percentiel exact `percentileRank(composiet, referentie)`? — zo niet, dan is het tegen een ándere referentie berekend dan het record beweert), niet-degeneraat (de referentie heeft echte spreiding), seizoensmatch (geen cross-seizoen-terugval) en overgevoeligheid (een extreem percentiel terwijl het composiet maar een fractie sd van het midden ligt = een fragiel cijfer dat hard zwaait op kleine bewegingen). De canary leest het verdict: `critical` (niet-reproduceerbaar of degeneraat = methodologie-inconsistentie) breekt de run rood en alarmeert; `degraded` (dun/cross-seizoen/overgevoelig) is een zichtbare notitie. Aanleiding: een onverwacht laag cijfer vereiste een handmatige check of het wel tegen een consistente referentie werd vergeleken; die controle hoort geautomatiseerd. Implementatie `app/engine/src/methodology/reference-audit.ts`, gepind in `reference-audit.test.ts` + `test_healthcheck.py`.
